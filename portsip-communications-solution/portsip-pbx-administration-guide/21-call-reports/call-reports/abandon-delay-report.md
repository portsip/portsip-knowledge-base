...
| **Abandoned Waiting ST2 ... Abandoned Waiting ST10**     | The total number of interactions that entered this queue and were subsequently abandoned while waiting in the queue within the service time interval bounded by the two indicated service time thresholds. If the lower service time threshold is not defined, this metric returns 0. If the upper service time threshold is not defined, this metric uses no upper limit for the service time interval. For example, **Abandoned Waiting ST2** is the total number of interactions that were abandoned within the interval bounded by the first and second service time thresholds.  |
