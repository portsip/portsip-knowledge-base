...
Each tenant can configure their own mail server to receive email notifications. Alternatively, if the system administrator allows it, tenants can use the system administrator’s mail server settings. For more details, refer to [_Apply Mail Settings to Tenants_](./#apply-mail-settings-to-tenants).
...
If the PBX system administrator has enabled the [**Apply Mail Settings to All Tenants**](./#apply-mail-settings-to-all-tenants) option, you will see the following text in green:\
...
If you prefer to configure your own email server settings for sending notifications, follow the guide [**Setting Up Mail Server**](./#setting-up-mail-server) to configure it. Once configured, the PBX will use your settings to send email notifications, even if the system administrator has enabled the **Apply Mail Settings to All Tenants** option.
