...
* Enhanced caller number masking in inbound rules with support for:
  ...
  * Wildcard patterns using `*`, where the pattern must match both the prefix and the total number length\
    (e.g., `0036***` matches numbers that start with `0036` and have a total length of 7 digits)
...
* Fixed an issue where auto-provisioned Gigaset phone information was displayed incorrectly in the web portal.
* Fixed an issue where an attended transfer between a trunk and a Snom M100 device could cause the trunk call to be disconnected.
* Fixed an issue where creating a tenant user named `admin` could prevent the system administrator from subscribing to global WSI events (`global_*`).
