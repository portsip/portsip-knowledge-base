...
PortSIP PBX has optimized the default log configuration for the Data Flow service, which is based on ClickHouse.
...
The Data Flow log rotation policy has now been improved. Logs are retained for one week by default, helping reduce disk usage and improve service reliability.
...
* Fixed an issue where SMS messages could fail to be parsed when a custom webhook URL was configured for a Twilio SMS trunk.
