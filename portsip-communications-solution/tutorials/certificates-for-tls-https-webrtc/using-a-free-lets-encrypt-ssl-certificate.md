...
PortSIP provides a Cert Manager service that allows you use Let's Encrypt SSL certifiate and renew it automatically. When a certificate is nearing expiration, the renewal process is handled automatically by the Cert Manager service. No user action is required, and existing services continue to operate securely during the renewal process.
...
* A domain name that resolves to the SBC server’s static public IP address(DNS A record)
...
Install the PortSIP Cert Manager
